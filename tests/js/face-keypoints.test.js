...
const noRot = new THREE.Quaternion();   // head facing +Z (forward), +X = left
...
describe('computeFaceKeypoints', () => {
    ...
    it('eyes sit forward (+Z) and above the head origin (no head rotation)', () => {
        const f = computeFaceKeypoints(head, neck, noRot);
        for (const e of [f.eye_L, f.eye_R]) {
            expect(e.z).toBeGreaterThan(head.z);
